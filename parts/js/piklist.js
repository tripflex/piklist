...
;(function($, window, document, undefined) 
{
  ...
  PiklistFields.prototype = {
        ...
        for (var i in field.conditions)
        {
          ...
          switch (type)
          {
            case 'update':
              ...
              $(document).delegate(':input[name="' + field.name + '"]', 'input propertychange change', this.conditions_handler(field_selector, field.conditions[i]));
            ...
            default:
              ...
              else
              {
                ...
                {
                  field_selector = '#' + (field.prefix ? '_' + field.field : field.id);
                }
                else
                {
                  field_selector = '#' + field.id;
                }                
              }
              
              if ($.inArray(field_selector + field.conditions[i].id, this.processed_conditions[type]) == -1)
              {
                $(document).delegate('.' + field.conditions[i].id, 'change', this.conditions_handler(field_selector, field.conditions[i]));
                $(':input:not(:radio)[class~="' + field.conditions[i].id + '"], :radio:checked[class~="' + field.conditions[i].id + '"]').trigger('change');

                this.processed_conditions[type].push(field_selector + field.conditions[i].id);
              }
    ...
    conditions_handler: function(selector, condition) 
    {
      return function()
      {
        var field = $(selector);
