...
class PikList_Validate
{
  ...
  {
    if (!isset($_REQUEST['piklist']['fields_id']) || !$data = get_transient(piklist::$prefix . $_REQUEST['piklist']['fields_id'])) 
    {
      return false;
    }
    ...
    foreach ($data as $type => &$fields)
    {
      ...
      {
        $field['valid'] = true;
        $name = isset($field['label']) && !empty($field['label']) ? $field['label'] : $field['field'];

        // Lets check to see if its required
        if ($field['required'] && (isset($field['request_value']) && empty($field['request_value'])))
        {
          $field['valid'] = false;
    ...
    self::set_errors($_REQUEST['piklist']['fields_id']);
