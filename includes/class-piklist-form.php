...
class PikList_Form
{
  ...
  public static function get_field_id($field, $scope = false, $index = false, $prefix = true)
  {
    ...
    if (self::is_widget() && (!$scope || ($scope && ($scope != piklist::$prefix && $field != 'fields_id'))))
    {
    ...
    switch ($type)
    {
      ...
      case 'user_meta': 
        ...
        if ($id)
        {
          ...
          return !empty($meta) || $meta == '0' ? (count($meta) == 1 && empty($meta[0]) && $meta[0] != '0' ? null : $meta) : (metadata_exists($meta_type, $id, $meta_key) ? null : (isset($field['value']) ? $field['value'] : null));
        }
  ...
  {  
    ...
    // piklist::pre('---------------');
    //   
    // piklist::pre($field['field']);
    // piklist::pre($field['value']);
    ...
    if (!$field['group_field'])
    {
      ...
      if (!$stored_value && !isset($field['attributes']['placeholder']) && !in_array($field['type'], self::$field_list_types))
      {
    ...
    foreach (array('post', 'comment', 'user', 'taxonomy', 'post_meta', 'comment_meta', 'user_meta', 'term_meta') as $builtin)
    {
      ...
      switch ($builtin)
      {
        ...
        case 'user_meta':
          ...
          foreach ($data as $key => $value) 
          {
            $old_meta = get_metadata($meta_type, $id, $key, true);
            ...
            {                  
              if (!piklist::is_flat($value) && !isset($value[0]))
              {
                if ($old_meta)
                {
                  foreach (current($old_meta) AS $old_meta_key => $old_meta_value) 
                  {
                    if (!array_key_exists($old_meta_key, $value)) 
                    {
                      $value[$old_meta_key] = $old_meta_value;
                    }
                  }
                }
      ...
      do_action('piklist_save_field', $builtin, $data, self::$fields[$builtin]);
    }
  ...
  public static function save_upload($files, $storage = array())
  {
    ...
    {
      if ($files['error'][$fid][0] === UPLOAD_ERR_OK)
      {
        $attach_id = media_handle_sideload(
                      array(
                        'name' => $file_name[0]
                        ,'size' => $files['size'][$fid][0]
                        ,'tmp_name' => $files['tmp_name'][$fid][0]
                      )
                      ,0
                    );
